...
import { 
  ...
  Clock, 
  CircleDollarSign,
  type LucideIcon // Added 'type' keyword to fix the SyntaxError
} from "lucide-react";
...
export function PGDetailPage() {
  ...
  const allGalleryImages = useMemo(() => {
    ...
    // Filter out nulls to satisfy TypeScript string[] requirement
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-10">
            ...
            <section>
              ...
              {reviews.length === 0 ? (
                <p className="text-stone-400 font-medium">No reviews yet. Be the first!</p>
              ) : (
          ...
          <aside className="space-y-6">
            <div className="sticky top-10">
              <Card className="p-8 border-2 border-stone-900 rounded-4xl shadow-2xl">
                ...
                {openWrite && <p className="mt-4 text-xs text-center text-teal-600 font-bold italic animate-pulse">Opening review dialog...</p>}
              </Card>

              <div className="mt-8 p-4 flex gap-3 text-stone-400 bg-stone-50 rounded-2xl">
                <AlertTriangle size={20} className="shrink-0" />
                <p className="text-[11px] leading-tight font-medium">
                  RateYourPG uses AI to summarize feedback. Always visit in person.
                </p>
              </div>
            </div>
          </aside>
