...
import { PenLine, Search, MapPin, School } from "lucide-react";
...
export function HomePage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      // 2. Fetch Areas (The new carousel data)
  ...
  const runExplore = useCallback(async () => {
    ...
    const allCurrent = [...colleges, ...areas];
    const localBest = allCurrent
    ...
    try {
      ...
      const { data: locRows } = await sb
        .from("locations")
        .select("*")
        .or(`name.ilike.%${raw}%,city.ilike.%${raw}%`)
        .limit(1);
  ...
  return (
    <>
          ...
          <div className="mt-10 flex max-w-2xl flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search by college (e.g. RVCE) or area (e.g. Koramangala)"
              />
            </div>
      ...
      <main className="mx-auto max-w-6xl space-y-16 px-4 py-16 sm:px-6">
        ...
        {/* SECTION: POPULAR COLLEGES CAROUSEL */}
        {colleges.length > 0 && (
          <section>
        ...
        {/* SECTION: POPULAR AREAS CAROUSEL */}
        {areas.length > 0 && (
          <section>
            <div className="mb-6 flex items-center gap-2">
