...
import { Filter, MapPin, SlidersHorizontal, Plus, Search as SearchIcon } from "lucide-react"; // Added Plus & SearchIcon
...
import { AddPgModal } from "@/components/pg/AddPgModal"; // Import your AddPgModal
...
export function LocationPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      const { data: loc } = await sb.from("locations").select("*").eq("slug", slug).maybeSingle();
      ...
      const { data: plist } = await sb.from("pg_with_ratings").select("*").eq("location_id", (loc as LocationRow).id);
  ...
  return (
    <>
        ...
        <div className="mt-6 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
            <div className="relative w-full sm:max-w-xs">
              <SearchIcon className="absolute left-3 top-2.5 text-stone-400" size={16} />
              <input
                className="w-full rounded-xl border border-stone-200 pl-9 pr-3 py-2 text-sm outline-none focus:ring-2 focus:ring-teal-500/20"
          ...
          <div className="space-y-4">
            {filtered.length > 0 ? (
              ...
              <div className="rounded-2xl border-2 border-dashed border-stone-200 py-16 text-center">
                ...
                <p className="mt-1 text-stone-600">Know a PG in {location.name}? Add it to help others!</p>
                <Button 
                  onClick={() => setAddPgOpen(true)}
      ...
      <AddPgModal 
        ...
        initialLocationId={location.id} // Pass current location ID
        onCreated={(newPg) => {
          // Optional: Add new PG to the list locally or refresh
