...
import { Filter, MapPin, SlidersHorizontal, Plus } from "lucide-react"; // Added Plus icon
...
import { AddPgForm } from "@/components/pg/AddPgForm"; // Import your form
    ...
    <>
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-teal-800">{location.city}</p>
          ...
          {/* Main "Add PG" Button */}
          <Button onClick={() => setIsAddPgOpen(true)} className="flex items-center gap-2">
            <Plus size={18} />
            Add a PG
          </Button>
        </div>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_320px]">
          <div className="space-y-4">
            ...
            {/* Improved Empty State with Add Button */}
            {filtered.length === 0 && (
              <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-stone-200 py-16 px-4 text-center">
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-stone-100 text-stone-400">
                  <Plus size={32} />
                </div>
                <h3 className="mt-4 text-lg font-bold text-stone-900">No PGs found here yet</h3>
                <p className="mt-2 text-stone-500 max-w-xs">
                  Be the first to help others by adding a PG near {location.name}.
                </p>
                <Button onClick={() => setIsAddPgOpen(true)} className="mt-6 bg-teal-600">
                  Add a PG in {location.name}
                </Button>
              </div>
            )}
          </div>
      ...
      {/* Modal containing the AddPgForm */}
      <Modal open={isAddPgOpen} onClose={() => setIsAddPgOpen(false)}>
        <AddPgForm 
          locationId={location.id}
          locationName={location.name}
          city={location.city}
          onCancel={() => setIsAddPgOpen(false)}
          onSuccess={(pg) => {
            setIsAddPgOpen(false);
            // Optionally refresh the list or navigate to the new PG
            window.location.href = `/pg/${pg.slug}?write=1`;
          }}
        />
      </Modal>
    </>
